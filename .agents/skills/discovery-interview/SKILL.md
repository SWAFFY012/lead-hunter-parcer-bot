...
1. **Chat Interaction:** You MUST conduct the entire interview, ask all questions, and provide all `AskUserQuestion` options exclusively in **Russian**. 
2. **Document Generation:** When you reach Phase 6, the final specification document generated in `thoughts/shared/specs/` MUST be written entirely in **Russian**.
3. **Internal Logic:** Keep your internal reasoning and adherence to these rules based on this English prompt, but your external output to the user must never be in English.
