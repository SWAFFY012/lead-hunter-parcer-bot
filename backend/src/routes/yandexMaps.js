...
router.post('/start', (req, res) => {
  if (getYandexMapsStatus().isRunning) {
    return res.status(409).json({ ok: false, error: 'Парсер Яндекс Карт уже работает в другой вкладке.' });
  }
  ...
  void startYandexMapsParsing({ query, targetCount, filters });
